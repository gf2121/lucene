...
BLOCK_SIZE = 128
BPVS = [1, 2, 4, 8, 12, 16, 20, 24, 28, 32, 40, 48, 56, 64]
...
def write_decoder(bpv, f):
  ...
  f.write('\n  static void decode%d(RandomAccessInput src, long offset, long[] dst) throws IOException {\n' % bpv)
  f.write('    long pos = offset;\n')
...
def write_round(bpv, long_per_round, round):
  ...
  for i in range(0, long_per_round):
    f.write('      long l%d = src.readLong(pos);\n' % (i))
    f.write('      pos += Long.BYTES;\n')
...
if __name__ == '__main__':
  ...
  for bpv in BPVS:
    f.write('      case %d:\n' % bpv)
    f.write('        decode%d(src, offset, dst);\n' % bpv)
    f.write('        break;\n')
  f.write('    }\n')
  f.write('  }\n')
