...
class Trie {
  ...
  long saveArcs(Node node, IndexOutput index, ByteBuffersDataOutput outputsBuffer, long startFP)
    ...
    for (PositionStrategy strategy : PositionStrategy.values()) {
      ...
      } else if (strategyCost == positionBytes
  ...
  enum PositionStrategy {
    ...
    BITS(0) {
      ...
      int lookup(int targetLabel, IndexInput in, int positionBytes, int minLabel)
        ...
        in.skipBytes(wordIndex << 3);
        long word = in.readLong(); // probably over-read.
        long mask = (1L << bitIndex);
        if ((word & mask) == 0) return -1;
        in.seek(fp);
        int pos = 0;
        for (int i = 0; i < wordIndex; i++) {
          pos += Long.bitCount(in.readLong());
        }
